    ...
    var article = {
        ...
        fetchRelatedArticle: function () {
            var relatedArticle = $.ajax({
                url: host + 'focus/getRelevantArticleList.htm?type=1&clientType=pc',
                type: 'POST',
                dataType: 'JSON'
            });
        ...
        fillRelatedArticle: function (data) {
            ...
            var listItems = relatedArticleData.map(function (item) {
                return '<p class="articles-list-title "><a href="../focus_detail/focus_detail.html?id=' + item.id + '" class="list-title" target="_blank">'+item.focusTitle+'</a></p>'
            });
