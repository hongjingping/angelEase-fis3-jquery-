...
//		    icon.className = 'glyphicon glyphicon-plus';
	...
	/*$.ajax({
		...
		success: function (result) {
			if (result.code == '9999' || (result.code == '0000' && result.data.investorInfo.status === '')) {
			...
			}else if(result.code == '0000' && result.data.investorInfo.status !== ''){
	...
	$('#investorbtn').on('click',function () {
		...
		if($('#agreement').val() == '0'){
			...
			if ($('input[type="radio"]:checked').val() == '0') {
				if($('#isinves').val() == '0') {
					var positionLimitAlert = new SmartAlert({
						title: '',
						content: '<i class="ae-icon ae-icon-attention notice-attention"></i><span class="content-txt">抱歉，根据相关政策，合格投资人必须为非现役军人、非公务员</span>',
						type: 'confirm',
						okText: '我知道了',
						maskClosable: false,
					});
					positionLimitAlert.open();
					return;
				}
				investorformSubmit();
			...
			success: function(result) {
				if(result.code == '0000') {
					...
					var ageLimitAlert = new SmartAlert({
						title: '年龄超限',
                        content: '<i class="ae-icon ae-icon-attention alert-notice-attention"></i><div class="content-txt-wrapper"><span class="content-txt">'+ result.desc +'</span></div>',
						type: 'confirm',
						okText: '我知道了',
						maskClosable: false,
					});
...
/*function delimg(id){
    ...
    input.addEventListener('change', function (event) {
    	var file = event.target.files[0];
		imageBase64(file).then(function (src) {
			img.src = src;
    ...
    Array.prototype.forEach.call(imageUploadComps, function (comp) {
    	if(comp.getAttribute('data-id') == id){
